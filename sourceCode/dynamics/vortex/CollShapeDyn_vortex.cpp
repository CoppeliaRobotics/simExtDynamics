...
CCollShapeDyn_vortex::CCollShapeDyn_vortex(CDummyShape* shape,CDummyGeomProxy* geomData,Vx::VxUniverse* universe)
{ // This is the constructor of a collision shape wrapper. The wrapper can contain several collision objects grouped in a compound
    _geomData=geomData;
...
//    const int materialUniqueId=intParams[7];
    ...
    bool isPrimitive=(primType!=sim_pure_primitive_none);
    bool isActuallyPrimitive=isPrimitive;
    if (treatPureShapesAsConvexShapes&&(primType!=sim_pure_primitive_heightfield))
        isPrimitive=false;
    if (isPrimitive)
    { // We have a pure primitive here. It is either a simple collision objects, or a compound of simple collision objects.
        ...
        { // we have a SIMPLE pure shape. This is a single simple collision object (e.g. sphere, cuboid, cylinder or heightfield)
            ...
            float hollowScaling=_simGetPureHollowScaling((CDummyGeometric*)geomInfo); // this represents a value between 0.0 and 0.9999. When multiplying the shape dimensions with that value, we obtain the negative shape dimensions (if 0.0, there is no negative shape).
            int loopCnt=1;
            if (hollowScaling!=0.0f)
            {
                loopCnt=2;
...
CCollShapeDyn_vortex::~CCollShapeDyn_vortex()
{
    for (int i=0;i<int(_vortexGeoms.size());i++)
    {
    ...
    for (int i=0;i<int(_vortexMmeshVertices_scaled.size());i++)
        delete _vortexMmeshVertices_scaled[i];
    for (int i=0;i<int(_vortexMconvexPlanes_scaled.size());i++)
        delete _vortexMconvexPlanes_scaled[i];
    for (int i=0;i<int(_vortexMconvexPolygons.size());i++)
        delete _vortexMconvexPolygons[i];
}

Vx::VxGeometry* CCollShapeDyn_vortex::_createVortexSimpleGeometry(int pType, const C3Vector& ins,float hollowScaling, CDummyGeomWrap* geomInfo, float linScaling)
{
...
Vx::VxTriangleMeshUVGrid* CCollShapeDyn_vortex::_createVortexUVGridMesh(float* allVertices, int allVerticesSize, int* allIndices, int allIndicesSize, float linScaling)
{
...
Vx::VxTriangleMeshBVTree* CCollShapeDyn_vortex::_createVortexBVTreeMesh(float* allVertices, int allVerticesSize, int* allIndices, int allIndicesSize, float linScaling)
{
    Vx::VxTriangleMeshBVTree* bvTreeMesh = new Vx::VxTriangleMeshBVTree(allIndicesSize/3);
...
Vx::VxCollisionGeometry* CCollShapeDyn_vortex::_createVortexCollisionGeometry(Vx::VxUniverse* universe,CDummyGeomWrap* geomInfo,Vx::VxGeometry* vxGeometry,const Vx::VxTransform& vxTransform,const float* floatParams,const int* intParams)
{
    const double frictionCoeff_primary_linearAxis=getVortexUnsignedDouble(floatParams[0]);
    const double frictionCoeff_secondary_linearAxis=getVortexUnsignedDouble(floatParams[1]);
    const double frictionCoeff_primary_angularAxis=getVortexUnsignedDouble(floatParams[2]);
...
Vx::VxCollisionGeometry* CCollShapeDyn_vortex::getVortexGeoms(int index)
