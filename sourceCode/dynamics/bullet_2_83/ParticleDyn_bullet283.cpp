...
CParticleDyn_bullet283::CParticleDyn_bullet283(const C3Vector& position,const C3Vector& velocity,int objType,float size,float massOverVolume,float killTime,float addColor[3]) : CParticleDyn(position,velocity,objType,size,massOverVolume,killTime,addColor)
{
}

CParticleDyn_bullet283::~CParticleDyn_bullet283()
{
}

bool CParticleDyn_bullet283::addToEngineIfNeeded(float parameters[18],int objectID)
{
	if (_initializationState!=0)
		return(_initializationState==1);
	_initializationState=1;

	CRigidBodyContainerDyn_bullet283* rbc=(CRigidBodyContainerDyn_bullet283*)(CRigidBodyContainerDyn::currentRigidBodyContainerDynObject);
...
void CParticleDyn_bullet283::handleAntiGravityForces_andFluidFrictionForces(const C3Vector& gravity,float linearFluidFrictionCoeff,float quadraticFluidFrictionCoeff,float linearAirFrictionCoeff,float quadraticAirFrictionCoeff)
{
...
void CParticleDyn_bullet283::removeFromEngine()
{
...
void CParticleDyn_bullet283::updatePosition()
{
	if (_initializationState==1)
	{
		float linScaling=CRigidBodyContainerDyn::getPositionScalingFactorDyn(); 
